...
def test_mount_valid_images(test_client, expected_test_results, test_image_path):
    ...
    if not os.path.isfile(os.path.join("test_images", test_image_path)):
        pytest.skip(
            f"Skipping disk mounting service test because test disk image is missing ({test_image_path})"
        )
    ...
    for disk_image_info in expected_test_results:
        if disk_image_info["expected_json_response"]["imagepath"] == test_image_path:
            ...
            print(f"mounting: {relative_image_path}")
            response = test_client.put(
                f"/mounts/{relative_image_path}?mount_dir={mount_dir}"
            )
            assert response.status_code == 200
            ...
            for volume in volumes:
                volume.pop(
                    "mountpoint"
                )  # removing volume mountpoints because values are randomized
            ...
            del response_json[
                "mountpoint"
            ]  # removing disk mountpoint because value is randomized

            print(f"disk_mountpoint: {disk_mountpoint}")
            ...
            response = test_client.get(f"/mounts/{relative_image_path}")
            assert response.status_code == 200
            ...
            all_volume_mountpoints = [
                x["mountpoint"] for x in volumes if x["mountpoint"] != ""
            ]
            for volume in volumes:
            ...
            response = test_client.delete(f"/mounts/{relative_image_path}")
            assert response.status_code == 200
...
def test_mount_images_with_unmountable_volumes(
    test_client, expected_test_results, test_image_path
):
    """
    GIVEN a Thumbtack Flask application client
    ...
    for disk_image_info in expected_test_results:
        if disk_image_info["expected_json_response"]["imagepath"] == test_image_path:
            ...
            if not os.path.isfile(os.path.join("test_images", relative_image_path)):
                ...
                pytest.skip(
                    f"Skipping disk mounting service test because test disk image is missing ({relative_image_path})"
                )

            # MOUNT IMAGE
            # Verify PUT /mounts/<TEST_DISK_IMAGE> returns the new mount
            print(f"mounting: {relative_image_path}")
            response = test_client.put(f"/mounts/{relative_image_path}")
            assert response.status_code == 400
